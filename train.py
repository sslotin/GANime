...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    fake = (torch.cuda.FloatTensor(np.ones(validity.data.shape))).requires_grad_()
...
for epoch in range(opt.n_epochs):
    for i, (batch_A, batch_B) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            print("Epoch %d/%d | Batch %d/%d | D loss: %f | G loss: %f | cycle: %f | ETA: %s"
                             % (epoch, opt.n_epochs, i, total, D_loss.item(),
                                G_adv.item(), G_cycle.item(), time_left))
